...
///  selective invalidation (dependency_resolution: fast_slow_caching)
...
///  "ModelSpeeds" option, set directly under this scanner's own yaml block (Core has no
///  notion of model or parameter speeds at all). Each model entry can be either a plain
...
scanner_plugin(fast_slow_test, version(1, 0, 0))
{
    ...
    int plugin_main ()
    {
        ...
        {
            // Draw the whole point (slow and fast parameters alike) uniformly at random.
            ...
            // Repeat the same point, redrawing only the fast parameters each time.
