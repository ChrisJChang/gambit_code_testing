...
namespace Gambit
{
  ...
  namespace DRes
  {
    ...
    void DependencyResolver::doResolution()
    {
      ...
      // Precompute, for each (model, parameter) pair, which vertices are sensitive to it, for
      // fast-slow selective staleness marking.
    ...
    void DependencyResolver::computeParameterStaleSets()
    {
      ...
      // Identify which vertex is which model's primary model functor, and the full parameter
      // list of each active model (used as the default sensitivity when a consumer of a model
      // hasn't declared a narrower subset via ALLOW_MODEL_PARAMETERS).
      ...
      for (const auto& model_and_functor : boundCore->getActiveModelFunctors())
      {
        ...
        modelAllParams[model_name] = model_functor_ptr->getcontentsPtr()->getKeys();
      ...
      // Single forward pass over the topologically-sorted graph (providers before consumers),
      // so that every in-edge's sensitivity set is already known by the time we reach a vertex.
      for (const VertexID& v : function_order)
      {
        ...
        else
        {
          ...
          for (std::tie(ei, ei_end) = in_edges(v, masterGraph); ei != ei_end; ++ei)
          {
            ...
            {
              // Fed directly by a model's primary model functor (the implicit ALLOW_MODELS
              // dependency): use the declared parameter subset, or all of the model's
              // parameters if no subset was declared.
              const str& model_name = prov_model_it->second;
              if (masterGraph[v]->hasDeclaredModelParameters(model_name))
              {
                for (const str& p : masterGraph[v]->getDeclaredModelParameters(model_name))
                  sensitivity.insert({model_name, p});
              }
              else
              {
                for (const str& p : modelAllParams[model_name]) sensitivity.insert({model_name, p});
            ...
            else
            {
              ...
              if (sit != vertexSensitivity.end())
                sensitivity.insert(sit->second.begin(), sit->second.end());
            }
          }
        }
    ...
    /// Mark for recalculation only those active functors sensitive to one of the given (changed)
    /// (model, parameter) pairs, plus any functor that always requires recalculation.
    void DependencyResolver::markStaleForChangedParameters(const std::set<std::pair<str,str>>& changed_params)
    {
      ...
      // regardless of whether any parameter they depend on actually changed this point.
      for (std::tie(vi, vi_end) = vertices(masterGraph); vi != vi_end; ++vi)
      {
